...
//acc and gyro calib
static const float ACC_BIAS_MSS[3]   = {-0.10836936f, -0.14618062f, -0.24424600f}; //IMU wetlink -0.04100180f, -0.09475613f,  0.10522700f
static const float ACC_SCALE[3]      = { 1.00371170f, 0.99903278f, 0.98581686f}; //IMU wetlink  0.99969568f,  1.00054180f,  0.98821163f

// Optional fallback gyro bias (rad/s). Prefer per-boot bias.
static const float GYRO_BIAS_RADS_FALLBACK[3] = {-0.00458946f, 0.00458547f, -0.01216408f}; //IMU wetlink -0.01102138f, -0.01515991f,  0.01481001f

static const uint32_t OUTPUT_RATE_HZ = 240;
...
void loop() {
  uint32_t now = micros();
    ...
    uint32_t end = micros();
    Serial.print("no writing"); Serial.println(end - now);
  ...
  if ((int32_t)(now - previous) >= OUTPUT_PERIOD_US) {
    ...
    ImuPayload p{ax, ay, az, gx, gy, gz};
    ...
    Serial.print("writing time"); Serial.println(e2-s2);
  }
