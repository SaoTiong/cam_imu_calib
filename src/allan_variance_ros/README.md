...
### Streaming a calibrated Arduino IMU

If you calibrated an Arduino-based ICM20948 IMU and it prints CSV lines as `ax, ay, az, gx, gy, gz` (m/s^2 and rad/s), you can stream it into ROS directly:

1. Connect the Arduino and identify the serial port (e.g. `/dev/ttyACM0`).
2. Run the provided node:\
   ``rosrun allan_variance_ros icm20948_serial_node.py _port:=/dev/ttyACM0 _topic:=/icm20948/imu _frame_id:=icm20948_link``
3. (Optional) Set `_angular_velocity_variance` and `_linear_acceleration_variance` to embed the best-known noise values in the `sensor_msgs/Imu` covariance fields.
4. Record the topic for **at least 3 hours** at 480 Hz to pair with a 60 FPS camera:\
   ``rosbag record -O icm20948_480hz.bag /icm20948/imu``
5. Analyze the bag with the included config `config/icm20948_serial.yaml` to generate the Kalibr-compatible `imu.yaml`.
